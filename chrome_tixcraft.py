...
def get_config_dict(args):
    ...
    if os.path.isfile(config_filepath):
        ...
        with open(config_filepath, "rb") as json_data:
            config_dict = orjson.loads(json_data.read())
...
def get_chrome_options(webdriver_path, config_dict):
    ...
    print(f'\033[32mSet up normal chrome options\033[0m')
    ...
    chrome_options.add_argument("--disable-features=EnablePrivacySandbox,IsolateOrigins,site-per-process,TrackingPrevention,TranslateUI,PrivacySandboxSettings4")
...
def get_uc_options(uc, config_dict, webdriver_path):
    ...
    print('\033[32mSet up uc chrome options\033[0m')
    ...
    # options.add_argument("--disable-extensions")  required for checkbox
    # for brave
    options.add_argument("--disable-features=EnablePrivacySandbox,IsolateOrigins,site-per-process,TrackingPrevention,TranslateUI,PrivacySandboxSettings4")
...
def get_driver_by_config(config_dict):
    ...
    if config_dict["browser"] in ["chrome","brave"]:
        ...
        if config_dict["webdriver_type"] == CONST_WEBDRIVER_TYPE_SELENIUM:
            print(f'\033[32mExecute chrome_tixcraft.py line 630\033[0m')
        ...
        if config_dict["webdriver_type"] == CONST_WEBDRIVER_TYPE_UC:
            ...
            if platform.system().lower()=="windows":
                if hasattr(sys, 'frozen'):
                    ...
                    freeze_support()
            print(f'\033[32mExecute chrome_tixcraft.py line 639\033[0m')
        ...
        try:
            ...
            if 'kktix.c' in homepage:
                if len(config_dict["advanced"]["kktix_account"])>0:
                    # for like human.
                    try:
                        print(f'\033[32mExecute chrome_tixcraft.py line 781\033[0m')
                        driver.get(homepage)
                        time.sleep(5)
    ...
    if not mapSelectArea is None:
        ...
        if len(zone_string) > 0:
            ...
            try:
                ...
                zone_info = orjson.loads(zone_string)
                if show_debug_message:
...
def get_performance_log(driver, url_keyword):
    ...
    try:
        ...
        for log in logs:
            ...
            network_log = orjson.loads(log["message"])["message"]
